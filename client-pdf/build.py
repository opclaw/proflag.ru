...
def s04(s, page, total):
    ...
    s.para(x0 + 48, y0 + 500,
           ...
           "Спрос по направлению «печать на ткани» в Москве и области — 1 500–3 000 обращений в месяц: "
           "сейчас этот поток просто обрывается.",
    ...
    s.text(M + 110, yb + 32,
           "Самый быстрый и самый дешёвый результат проекта: обе потери убираются на первой неделе.",
           28, "700", INK)
...
def s05(s, page, total):
    ...
    s.text(rx + 80, yy + 24, "Каждый месяц промедления — минус ~60 точек входа в выдаче.", 25, "800", RED)
...
def s07(s, page, total):
    ...
    s.para(M + 48, yb + 30,
           ...
           "и это самая быстро окупаемая часть работ.",
...
def s10(s, page, total):
    ...
    s.head("Как мы это чиним: 4 этапа за 90 дней",
           "Ниже — что делаем и какой результат получаем на каждом этапе.")
    ...
    phases = [
        ...
        ], "Управляемый рост и окупаемость канала"),
    ]
    y0, hh = 392, 740
    cw = (CW - 3 * 36) / 4
    for i, (days, ttl, col, soft, items, result) in enumerate(phases):
        ...
        s.text(x0 + 60, ry + 22, "РЕЗУЛЬТАТ ЭТАПА", 17, "800", MUTED, ls=2.0)
...
def s11(s, page, total):
    s.chrome(page, total)
    s.head("Что это даёт в цифрах",
           "Прогноз строится на устранении найденных потерь и готовом семантическом ядре.")

    y0, hh = 392, 608
    lw = 1180
    s.rrect((M, y0, M + lw, y0 + hh), r=22, fill=WHITE, outline=LINE, width=2)
    s.text(M + 48, y0 + 44, "Органический трафик, индекс (сейчас = 100)", 30, "800", INK)

    cx0, cy0 = M + 120, y0 + 130
    cx1, cy1 = M + lw - 70, y0 + 400
    vmin, vmax = 80, 430
    vals = [100, 150, 250, 320, 400]
    labs = ["сейчас", "3 мес", "6 мес", "9 мес", "12 мес"]
    for gv in (100, 200, 300, 400):
        gy = cy1 - (cy1 - cy0) * (gv - vmin) / (vmax - vmin)
        s.line((cx0, gy), (cx1, gy), (238, 242, 248), 2)
        s.text(cx0 - 20, gy - 12, str(gv), 20, "600", MUTED, anchor="ra")
    pts = []
    for i, v in enumerate(vals):
        px = cx0 + (cx1 - cx0) * i / (len(vals) - 1)
        py = cy1 - (cy1 - cy0) * (v - vmin) / (vmax - vmin)
        pts.append((px, py))
    s.poly(pts + [(pts[-1][0], cy1), (pts[0][0], cy1)], (238, 244, 253))
    for i in range(len(pts) - 1):
        s.line(pts[i], pts[i + 1], BLUE, 5)
    for i, (px, py) in enumerate(pts):
        s.circle(px, py, 10, fill=WHITE, outline=BLUE, width=5)
        s.text(px, py - 52, "×%.1f" % (vals[i] / 100) if i else "100", 22, "800", BLUE, anchor="ma")
        s.text(px, cy1 + 22, labs[i], 21, "600", MUTED, anchor="ma")
    s.line((cx0, cy1), (cx1, cy1), (214, 222, 234), 3)

    s.para(M + 48, y0 + 480,
           "Это план, а не гарантия: замеряем метрики ежемесячно (Вебмастер, Метрика, Топвизор) "
           "и корректируем ход работ. Первые изменения видны уже через 2–3 недели.",
           24, "600", BODY, maxw=lw - 96, lh=1.42)

    rx = M + lw + 60
    rw = W - M - rx
    s.rrect((rx, y0, W - M, y0 + hh), r=22, fill=PANEL, outline=LINE, width=2)
    s.text(rx + 44, y0 + 44, "Целевые показатели через 12 месяцев", 27, "800", INK)
    tiles = [
        ("×4", "органический трафик к уровню старта", ""),
        ("400+", "запросов в топ-10 Яндекса по Москве", ""),
        ("+200%", "заявок, звонков и чатов с сайта", ""),
        ("1 200+", "страниц в индексе вместо 290", ""),
    ]
    ty = y0 + 116
    for big, small, _x in tiles:
        s.rrect((rx + 44, ty, W - M - 44, ty + 104), r=18, fill=WHITE, outline=LINE, width=2)
        s.text(rx + 76, ty + 24, big, 46, "900", BLUE)
        s.para(rx + 76 + s.tw(big, 46, "900") + 22, ty + 40, small, 23, "600", BODY,
               maxw=rw - 130 - s.tw(big, 46, "900"), lh=1.3)
        ty += 120

    # нижняя таблица динамики
    ty = 1032
    s.rrect((M, ty, W - M, ty + 178), r=20, fill=WHITE, outline=LINE, width=2)
    cols = [900, 300, 300, 300, 340]
    xs = [M + 44]
    for c_ in cols[:-1]:
        xs.append(xs[-1] + c_)
    hdr = ["МЕТРИКА", "СЕЙЧАС", "3 МЕС", "6 МЕС", "12 МЕС"]
    for x, t in zip(xs, hdr):
        s.text(x, ty + 26, t, 19, "800", MUTED, ls=2.2)
    data = [
        ("Страниц в индексе Яндекса", "290 ↓", "500+", "800+", "1 200+"),
        ("Запросов в топ-10 (Москва)", "десятки", "100+", "250+", "400+"),
        ("Заявок с сайта в месяц", "база", "+50%", "+100%", "+200%"),
    ]
    ry = ty + 66
    for i, row in enumerate(data):
        if i:
            s.line((M + 44, ry - 12), (W - M - 44, ry - 12), (240, 243, 248), 2)
        for j, (x, v) in enumerate(zip(xs, row)):
            if j == 0:
                s.text(x, ry, v, 24, "600", INK)
            else:
                s.text(x, ry, v, 24, "800", RED if v.endswith("↓") else (BLUE if j == 4 else BODY))
        ry += 40
...
def s12(s, page, total):
    ...
    cards = [
        ("−64 страницы", "каждый месяц", "Индекс Яндекса продолжает сокращаться: сайт теряет около 60 точек входа в выдачу ежемесячно.", RED, RED_SOFT),
    ...
    s.para(M, 1112,
           "SEO-эффект накапливается 2–3 месяца. Чтобы снять пик спроса 9 Мая, стартовать нужно "
           "в январе–феврале; чтобы успеть к выборным кампаниям — ещё раньше.",
...
def s13(s, page, total):
    ...
    steps = [
        ("Согласовать этап 1", "Договор и счёт. Старт работ в течение трёх рабочих дней, первые результаты — через две недели."),
    ...
    s.para(rx + 44, y0 + 320 - 24, "Эти материалы ведём мы — в PDF они намеренно не раскрываются.",
    ...
    s.para(M, 1096,
           "Полный комплект рабочих материалов — аудит на 12 разделов, план из 30 задач с исполнителями, "
           "ТЗ разработчику, семантическое ядро на 560+ запросов и контент-план — передаётся в проект после старта "
           "и остаётся в контуре работ.",
           24, "600", BODY, maxw=CW, lh=1.42)
